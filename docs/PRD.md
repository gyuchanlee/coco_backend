...
| PATCH | `/api/v1/tour-course/{courseId}/title` | 코스 제목 수정 (인증 필요) | ✅ |
...
| 코스 제목 수정 | ✅ 로그인 필요 | 미인증 → 401 / 타인 코스 → 403 |
...
| OQ14 | **코스 제목 저장**: `tour_course_user_defined.title VARCHAR(255)` 컬럼 추가 완료. `PATCH /{courseId}/title` 제목 수정 API 구현 완료 | ✅ |
