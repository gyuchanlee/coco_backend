...
- **⚠️ 스키마 갭**: 저장된 코스에 `title` 없음(BOQ10). POI별 예산 오버라이드 저장 불가(BOQ9).
...
### CO3. 사용자 코스 목록 조회
- **설명**: 로그인 사용자의 저장 코스 목록 반환 (코스 ID·기간·인원·이동수단·테마 요약). ⚠️ 현재 스키마에 `title`이 없어 코스 이름 대신 기간·인원으로만 식별해야 함(BOQ10).
...
| BOQ10 | `tour_course_user_defined.title` 컬럼 추가 여부 | CO3 코스 목록 표시 |
